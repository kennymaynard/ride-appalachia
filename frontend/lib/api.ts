...
export async function buildExploreAiPlan(payload: Record<string, unknown>): Promise<{source:string;message:string;stops:{destination_id:number;day:number;notes:string}[]}> { const response=await apiFetch("/api/explore/plan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!response.ok)throw new Error("Unable to build this plan");return response.json(); }
