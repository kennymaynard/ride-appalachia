...
import { createAdminEvent, getAdminEvents, moderateAdminEvent, updateAdminEvent } from "../lib/api";
import type { RideEvent } from "../lib/types";

const blank = { title: "", organizer: "", description: "", state: "KY", city: "", venue: "", address: "", latitude: "", longitude: "", start_date: "", end_date: "", category: "group_ride", vehicle_types: [] as string[], official_url: "", registration_url: "", facebook_url: "", image_url: "", verification_source: "", is_verified: false, is_featured: false, status: "pending", admin_notes: "" };
...
export function AdminEventsPanel({ adminPassword }: { adminPassword: string }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const payload = { ...form, latitude: form.latitude ? Number(form.latitude) : null, longitude: form.longitude ? Number(form.longitude) : null, end_date: form.end_date || form.start_date } as unknown as Partial<RideEvent>;
  ...
  function edit(item: RideEvent) {
    ...
    setForm({ title: item.title, organizer: item.organizer, description: item.description, state: item.state, city: item.city, venue: item.venue, address: item.address, latitude: item.latitude == null ? "" : String(item.latitude), longitude: item.longitude == null ? "" : String(item.longitude), start_date: item.start_date, end_date: item.end_date, category: item.category, vehicle_types: item.vehicle_types, official_url: item.official_url, registration_url: item.registration_url, facebook_url: item.facebook_url, image_url: item.image_url, verification_source: item.verification_source, is_verified: item.is_verified, is_featured: item.is_featured, status: item.status, admin_notes: item.admin_notes || "" });
  }
