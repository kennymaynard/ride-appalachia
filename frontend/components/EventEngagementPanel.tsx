...
export function EventEngagementPanel({ event }: { event: RideEvent }) {
  ...
  async function save() { try { const next = !engagement.saved; await saveRideEvent(event.id, token, next); setEngagement((current) => ({ ...current, saved: next })); } catch (e) { setNotice(e instanceof Error ? e.message : "Unable to save"); } }
  ...
  return <section className="dashboard-card" aria-label="Event actions">
    <div className="hero-actions">
      ...
      <a href={`/api/events/${encodeURIComponent(event.slug)}/calendar.ics`}>Add to Calendar</a>
